KioskBoard.Init({
	...
	theme: 'dark', // The theme of keyboard => "light" || "dark" || "flat" || "material" || "oldschool"
...
function playGame() {
	if ($("#name_input").val() != '') { // User name is not empty then start the game
		...
		location.href = "#page_2"; // Go to game description page, the very first start page of the game
		timer.start();
...
function gameFailed() {
	var audio = new Audio('audio/end_of_game.mp3'); // init "end of game.wav" audio to play
	backgroundAudio.pause(); // pause background audio to play another audio
